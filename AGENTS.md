...
6. `Shared/Playback/AudioPlayerManager.swift`
...
- Playback compartido: `Shared/Playback`
...
- iOS envia audio al watch con `WCSession.transferFile`, metadata ligera, artwork optimizado/omitido y limite de 200 MB.
...
- El motor compartido de reproduccion es `AudioPlayerManager`; soporta audio local y radio con `isLiveStream`.
- La radio del watch vive hoy en `Shared/UI/ContentViewWatch.swift`.
- La radio del iPhone vive en `RetroMusicAppiOS/RetroMusicAppiOS/RadioListView.swift` y usa `AudioPlayerManager` con stream directo.
...
- La radio iOS/watch usa `AudioPlayerManager` en modo `isLiveStream`, sin seek ni duracion fija.
...
- Catalogo de radio centralizado en `Shared/Models/RadioStation.swift`; revisar ahi antes de tocar vistas.
...
- La radio live expone buffering/error y hace reintentos cortos, pero sigue necesitando validacion en red real del reloj.
...
- Mantener el limite de 200 MB y mostrar errores visibles al usuario.
...
- Para radio/live stream, extender `AudioPlayerManager`; no volver al flujo Safari/web como reproductor principal.

5. Radio
- El catalogo de emisoras vive en `Shared/Models/RadioStation.swift`.
- Para bugs de radio en watch, depurar primero la ruta del reloj y no asumir que iOS reproduce con las mismas condiciones de red.
...
- Verificar que la URL usada viene de `RadioCatalog`.
- Inspeccionar estados de buffering/error y reintentos antes de cambiar la arquitectura.
...
- `retromusic-watch-sync-audit`: valida `transferFile`, limite de 200 MB, rutas JSON, `storedFileName`, upsert y borrado fisico.
- `retromusic-watch-autonomous-debug`: separa fallos de libreria local, radio y dependencia accidental del iPhone.
- `retromusic-radio-stream-validation`: revisa `RadioCatalog`, comprueba stream real, formato, TLS/ATS y feedback de error.
...
- Verificar que archivos mayores de 200 MB se rechazan con mensaje visible.
