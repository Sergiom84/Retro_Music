...
struct AudioTrack: Identifiable, Codable, Equatable {
    ...
    var filePath: URL {
        get {
            // Si storedFileName contiene una ruta absoluta, usarla; de lo contrario, construir desde documents
            if storedFileName.hasPrefix("/") {
                return URL(fileURLWithPath: storedFileName)
            }
            return Self.documentsDirectory.appendingPathComponent(storedFileName)
        }
        set {
            // Guardar solo el nombre del archivo si está en el directorio de documentos
            // De lo contrario, guardar la ruta completa
            let docPath = Self.documentsDirectory.path
            let newPath = newValue.path
            if newPath.hasPrefix(docPath) {
                storedFileName = newValue.lastPathComponent
            } else {
                storedFileName = newPath
            }
        }
    }
